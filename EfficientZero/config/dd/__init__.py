...
from core.utils import make_dd
from core.dataset import Transforms
from .env_wrapper import AtariWrapper
from .model import EfficientZeroNet


class DDConfig(BaseConfig):
    def __init__(self):
        super(DDConfig, self).__init__(
            training_steps=100000,
            last_steps=20000,
            test_interval=10,#10000,
        ...
        self.probe_env = make_dd('blabla', skip=0, max_episode_steps=500)
        self.obs_shape = self.probe_env.observation_space.shape
        self.action_shape = self.probe_env.action_space.n
    ...
    def set_game(self, env_name, save_video=False, save_path=None, video_callable=None):
        ...
        self.obs_shape = (self.obs_shape[0] * self.stacked_observations,) #(obs_shape[0] * self.stacked_observations, obs_shape[1], obs_shape[2])

        game = self.new_game()
        self.action_space_size = self.action_shape
    ...
    def new_game(self, seed=None, save_video=False, save_path=None, video_callable=None, uid=None, test=False, final_test=False):
        ...
        #     env = make_dd(self.env_name, skip=self.frame_skip, max_episode_steps=max_moves)
        # else:
        #     env = make_atari(self.env_name, skip=self.frame_skip, max_episode_steps=self.max_moves)

        # if self.episode_life and not test:
        #     env = EpisodicLifeEnv(env)
        env = make_dd(self.env_name, skip=self.frame_skip, max_episode_steps=500)
