...
class ClassicControlConfig(BaseMuZeroConfig):
    ...
    def make_dd(self, env_id, skip=0, max_episode_steps=None):
        class Args:
            def __init__(self):
                self.task = '1_1_1'
                self.crossing_style = 'minor'
                self.tasks = 'l'
                self.behavior_dist = False
                self.env_seed_offset = 0
                self.env_seeding_mode = 'train'
    ...
    def new_game(self, save_video=False, save_path=None, episode_trigger: Callable[[int], bool] = None, uid=None):
        env = self.make_dd(self.env_name) #, new_step_api=False)
