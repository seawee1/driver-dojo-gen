...
class Args:
    def __init__(self, task, offset, no_traffic):
        self.task = task
        self.env_seed_offset = offset
...
class MuZeroConfig:
    def __init__(self):
        ...
        args = Args('1_1_1', 0, False)
        self.env_config = get_env_config(args)

        global env_config
        env_config = self.env_config

        probe_env = DriverDojoEnv(_config=self.env_config)

        # print(probe_env.action_space)
        # print(probe_env.observation_space)

        ### Game
        self.observation_shape = (1, 1, probe_env.observation_space.shape[0])  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1,
        # length of array)
        self.action_space = list(range(probe_env.action_space.n))  # Fixed list of all possible actions. You should only edit the length
        probe_env.close()
        ...
        self.support_size = 10  # Value and reward are scaled (with almost sqrt) and encoded on a vector with a range of -support_size to support_size. Choose it so that support_size <= sqrt(max(
        # abs(discounted reward)))
        ...
        ### Training
        self.results_path = pathlib.Path(__file__).resolve().parents[1] / "results" / pathlib.Path(__file__).stem / datetime.datetime.now().strftime(
        ...
        self.checkpoint_interval = 100000  # Number of training steps before using the model for self-playing
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        ...
        self.env = DriverDojoEnv(_config=self.env_config)
        # if seed is not None:
        #     self.env.seed(seed)
    ...
    def step(self, action):
        ...
        observation = observation.reshape((1, 1, observation.shape[0]))
    ...
    def reset(self):
        ...
        """
        obs = self.env.reset()
        obs = obs.reshape((1, 1, obs.shape[0]))
        return obs
    ...
    def action_to_string(self, action_number):
        ...
        """
        actions = {}
        for i in self.env.action_space.n:
            actions[i] = 'Dummy val'
