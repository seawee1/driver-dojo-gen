...
class RepresentationNetwork(nn.Module):
    def __init__(
        ...
        #     nn.Linear(observation_shape[0], 512), nn.BatchNorm1d(512), nn.ReLU(),
...
class DynamicsNetwork(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTM(input_size=BLOCK_OUT_SIZE, hidden_size=self.lstm_hidden_size)
        self.bn_value_prefix = nn.BatchNorm1d(self.lstm_hidden_size, momentum=momentum)
        #print(self.lstm_hidden_size, NET_SIZE, fc_reward_layers)
        self.net_out = mlp(
    ...
    def forward(self, x, reward_hidden):
        ...
        value_prefix = self.net_out(value_prefix)
...
class PredictionNetwork(nn.Module):
    def __init__(
        ...
        #self.bn_value = nn.BatchNorm1d(BLOCK_OUT_SIZE, momentum=momentum)
        #self.bn_policy = nn.BatchNorm1d(BLOCK_OUT_SIZE, momentum=momentum)
        # self.fc_value = mlp(BLOCK_OUT_SIZE, fc_value_layers, full_support_size, init_zero=init_zero, momentum=momentum)
        # self.fc_policy = mlp(BLOCK_OUT_SIZE, fc_policy_layers, action_space_size, init_zero=init_zero, momentum=momentum)
...
class EfficientZeroNet(BaseNet):
    def __init__(
        ...
        fc_value_layers: list
            hidden layers of the value prediction head (MLP head)
        fc_policy_layers: list
            hidden layers of the policy predicion head (MLP head)
        ...
        self.porjection_in_dim = BLOCK_OUT_SIZE
