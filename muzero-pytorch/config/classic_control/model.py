...
class MuZeroNet(BaseMuZeroNet):
    def __init__(self, input_size, action_space_n, reward_support_size, value_support_size,
        ...
        self.hx_size = 512
        self.h_size = 512  # MINE
        self._representation = nn.Sequential(nn.Linear(input_size, self.hx_size),
                                             nn.Tanh())
        self._dynamics_state = nn.Sequential(nn.Linear(self.hx_size + action_space_n, self.h_size),
                                             nn.Tanh(),
                                             nn.Linear(self.h_size, self.hx_size),
                                             nn.Tanh())
        self._dynamics_reward = nn.Sequential(nn.Linear(self.hx_size + action_space_n, self.h_size),
                                              nn.LeakyReLU(),
                                              nn.Linear(self.h_size, reward_support_size))
        self._prediction_actor = nn.Sequential(nn.Linear(self.hx_size, self.h_size),
                                               nn.LeakyReLU(),
                                               nn.Linear(self.h_size, action_space_n))
        self._prediction_value = nn.Sequential(nn.Linear(self.hx_size, self.h_size),
                                               nn.LeakyReLU(),
                                               nn.Linear(self.h_size, value_support_size))
